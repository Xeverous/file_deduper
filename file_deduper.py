...
def make_entry(parent: Entry,
    ...
    if is_dir:
        ...
        size = stats.st_size
    return Entry(parent, path, name, is_dir, stats.st_mtime, size, checksum)
...
def walktree(root: Union[str, bytes, os.PathLike], parent: Entry, database: Database):
    for name in os.listdir(root):
        path = os.path.join(root, name)
        stats = os.lstat(path)
        if stat.S_ISDIR(stats.st_mode) or stat.S_ISREG(stats.st_mode):
            entry = make_entry(parent, path, name, stats)
            parent.children.append(entry)
            database.add(entry)
            if entry.is_dir:
                walktree(path, entry, database)
...
def scan(root_dirs: List[Union[str, bytes, os.PathLike]]):
    ...
    for root_dir in root_dirs:
        root_entry = make_entry(None, root_dir, os.path.basename(root_dir), os.lstat(root_dir))
        walktree(root_dir, root_entry, database)
    database.print()
...
# deduper
# depurator, rarefy-ier?, reducer, shaver, moderator
